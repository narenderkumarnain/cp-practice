...
void solve() {
    ...
    for(int i=0;i<n;i++) {
        ...
        tempratures[l]++;
        if(r < N) tempratures[r+1]--;
    }

    for(int i=1;i<N;i++) tempratures[i] += tempratures[i-1];

    tempratures[0] = tempratures[0] >= k ? 1 : 0;
    for(int i=1;i<N;i++) {
        int val = tempratures[i] >= k ? 1 : 0;
        tempratures[i] = val + tempratures[i-1];
    }
