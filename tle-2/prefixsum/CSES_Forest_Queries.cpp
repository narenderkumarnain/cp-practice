...
void solve() {
    ...
    vector<vector<int>> forrest(n, vector<int>(n, 0));
    ...
    // making it the prefix sum array
    for(int i=1;i<n;i++) forrest[0][i] += forrest[0][i-1];

    for(int row=1;row<n;row++) {
        int sm = 0;
        for(int col=0;col<n;col++) {
            sm += forrest[row][col];
            forrest[row][col] = sm + forrest[row-1][col];
        }
    }
    ...
    while(q--) {
        ...
        int sm = 0;
        sm += forrest[y2-1][x2-1];
        if(x1 > 1) sm -= forrest[y2-1][x1-2];
        if(y1 > 1) sm -= forrest[y1-2][x2-1];

        if(y1 != 1 && x1 != 1) sm += forrest[y1-2][x1-2];
        cout << sm << "\n";
