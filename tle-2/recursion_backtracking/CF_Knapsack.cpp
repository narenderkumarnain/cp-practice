...
int recur(int idx, int n, int w, vector<int> &weights, vector<int> &values) {
    if(idx == n) return 0;
    if(w <= 0) return 0;

    // choose it or not choose it
    int ans = max(
        recur(idx+1, n, w, weights, values), // not choosen
        w >= weights[idx] ? (recur(idx+1, n, w - weights[idx], weights, values) + values[idx]) : 0 // choosen
    );
...
int main() {
    ...
    cout << recur(0, n, w, weights, values) << endl;
    return 0;
}