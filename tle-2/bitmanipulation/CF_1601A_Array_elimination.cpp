...
int main() {
    ...
    while(t--) {
        ...
        for(int i=0;i<32;i++) {
            for(int j=0;j<n;j++) {
                if(arr[j] & (1 << i)) bitCount[i]++;
            }
        }

        int minBitcount = INT_MAX;
        for(int i=0;i<32;i++) if(bitCount[i] != 0) minBitcount = min(minBitcount, bitCount[i]);
        if(minBitcount == INT_MAX) minBitcount = n;
        vector<int> possibleK;
        for(int i=1;i<=minBitcount;i++) {
            bool valid = true;
            for(int j=0;j<32;j++) {
                if(bitCount[j] % i != 0) {
                    valid = false;
            ...
            if(valid) possibleK.push_back(i);
        }

        for(int i=0;i<possibleK.size();i++) cout << possibleK[i] << " ";
        cout << endl;
    }
    return 0;
}