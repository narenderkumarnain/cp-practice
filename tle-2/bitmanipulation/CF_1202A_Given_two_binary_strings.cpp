...
int main() {
    ...
    while(t--) {
        ...
        int revIdx = y.length() - first1OfY - 1;

        // cout << "revIdx: " << y << ":" <<revIdx << endl;

        int ans = 0;
        int n = x.length();
        int idx = n-1;
        while(idx >= 0) {
            if(x[idx] == '1' && (n - 1 - idx) - revIdx >= 0) {
                // cout << "choosing this ans: " << idx << " - " << n - 1 - idx - revIdx << endl;
                ans = (n - 1 - idx) - revIdx;
                break;
            }
            idx--;
