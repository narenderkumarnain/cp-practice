...
int main() {
    ...
    while(n != 1) {
        int sf = spf[n];
        allFactors.push_back(sf);
        n = n / sf;
        // if(sf == n) break;
    }

    // for(int i=0;i<allFactors.size();i++) cout << allFactors[i] << " , ";
    // cout << endl;

    if(allFactors.size() < k) {
        cout << -1 << endl;
    } else {
        int iter = allFactors.size() - k;
        int lastIdx = allFactors.size()-1;
        while(iter--) {
            allFactors[lastIdx-1] = allFactors[lastIdx-1] * allFactors[lastIdx];
            lastIdx = lastIdx-1;
        }

        for(int i=0;i<=lastIdx;i++) cout << allFactors[i] << " ";
        cout << endl;
    }
