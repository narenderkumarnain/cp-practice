...
int main() {
    ...
    while(t--) {
        ...
        int res = 1;
        for(int i=0;i<n;i++) {
            // add this to make all possible factors 
            if(x % arr[i] == 0) {
                // if(possibleFactors.find(arr[i]) != possibleFactors.end()) continue;
                set<int> newInsert;
                bool xPossible = false;
                for(auto &fac: possibleFactors) {
                    if((arr[i] < x) && ((1LL * fac) *  (1LL * arr[i] )== 1LL * x)) {
                        xPossible = true;
                        break;
                    }
                    if((arr[i] < x) && ((1LL * fac) *  (1LL * arr[i] ) < 1LL * x)) {
                        newInsert.insert(fac * arr[i]);
                    }
                }
                ...
                if(xPossible) {
                    ...
                    possibleFactors.insert(arr[i]);
                } else {
                    if(arr[i] < x) {
                        newInsert.insert(arr[i]);
                    }
                    for(auto &ab: newInsert) possibleFactors.insert(ab);
                }
            }
        }
