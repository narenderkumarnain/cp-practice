...
int main() {
    ...
    vector<bool> isOdd(n, true);

    int N = n+1;
    vector<int> twoCount(N, 0);
    twoCount[0] = 0;
    twoCount[1] = 0;
    // if(N > 2) 
    //     twoCount[2] = 1;
    for(int i=2;i<N;i++) {
            ...
            twoCount[i] = twoCount[i/2] + (i/2);
        
    }

    // for(int i=0;i<=n;i++) cout << i << ": " <<  twoCount[i] << endl;

    for(int i=0;i<n;i++) {
        // (n-1)! - i! and (n-i-1)! 
        int count = twoCount[n-1] - twoCount[i] - twoCount[n-i-1];
        if(count > 0) {
            isOdd[i] = false;
        }
    }
