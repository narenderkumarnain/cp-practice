...
lli exponentiation(lli a, lli b) {
    if(b == 1) return a;
    if(b == 0) return 1;

    lli val = exponentiation(a, b / 2);

    lli new_val = ((val % MOD) * (val % MOD)) % MOD;

    if(b % 2 != 0) {
        new_val = (new_val * (a % MOD)) % MOD;
    }
    return new_val;
}
...
int main() {
     ...
     * initial_value
     * 
     * ((initial_value % mod) * (a % mod)) % mod -> multiplication
     * 
     */
    int t;
    cin >> t;
