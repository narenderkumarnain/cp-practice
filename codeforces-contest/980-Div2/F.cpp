...
bool comparator(pair<double,double> a, pair<double,double> b) {
    if(a.second != b.second) return a.second > b.second;
    return a.first > b.first;
}

// map<int,pair<double,double>> dp; 

// pair<double,double> recur(vector<pair<double,double>> &vec, int idx, double currP, double currV) {
//     if(idx == vec.size()) return {1, 0};
//     if(dp.find(idx) != dp.end()) return dp[idx];

//     // choose it 

//     auto res = recur(vec, idx+1);

//     double tmp = (res.first * vec[idx].first) * (vec[idx].second + res.second);

//     double tmp2 = res.first * res.second;

//     if(tmp2 > tmp) {
//         dp[idx] = res;
//         return res;
//     } else {
//         dp[idx] = {res.first * vec[idx].first,vec[idx].second + res.second};
//         return dp[idx];
//     }
// }
...
void solve() {
    ...
    for(int i=0;i<n;i++) {
        double a, b;
        cin >> a >> b;
        vec[i] = {a / double(100), b};
    }

    sort(vec.begin(), vec.end(), greater<pair<double,double>>());
// 
    double currP = 1;
    double currV = 0;
    ...
    for(int i=0;i<n;i++) {
        double pi = vec[i].first;
        double vi = vec[i].second;

        double tmp = (pi * currP) * (vi + currV);
        double tmp2 = pi * vi;
    ...
    // auto res = recur(vec, 0);

    cout << setprecision(7) << fixed <<  (currP * currV) << endl;
}
