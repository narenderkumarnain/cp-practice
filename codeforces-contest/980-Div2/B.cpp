...
void solve() {
    ...
    while(t--) {
        ...
        while(cansDone < k) {
            // lli noOfZeros
            auto it = lower_bound(arr.begin(), arr.end(), iter);
            auto noOfZeros = std::distance(arr.begin(), it);
            // cout << "It: " << iter << " " << noOfZeros << endl;

            lli cansReq = k - cansDone;
            ...
            if(cansReq > (n - noOfZeros)) {
                cansDone += (n-noOfZeros);
                steps += n;
                iter++;
            } else {
                cansDone += (n-noOfZeros) - cansReq;
                steps += cansReq + noOfZeros;
                break;
            }
        }

        if(zeroSeq > 0) cout << 0 << endl;
        else cout << steps << "\n";
    }
}
