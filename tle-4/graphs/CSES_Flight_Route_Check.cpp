...
void solve()
{
    /*
    Algorithm
    if the graph has one strongly connected component, then valid else not

    we can use koseraju's algorithm
    - find topsort
    - find inverse graph
    - in order of topsort, find all connected components of graph
    */
    int n, m;
    cin >> n >> m;

    vector<int> graph[n], invgraph[n];
    for (int i = 0; i < m; i++)
    {
        int u, v;
        cin >> u >> v;
    ...
    // // int connectedCompsCount = 0;
    // vector<int> component(n, -1);
    // for(int i=0;i<n;i++) visited[i]=false;
    // function<void(int,int)> dfsinv = [&](int node, int comp){
    //     visited[node]=true;
    //     component[node]=comp;
    //     for(auto x: invgraph[node]){
    //         if(!visited[x]) dfsinv(x, comp);
    //     }
    // };

    // for(int i=0;i<n;i++){
    //     if(!visited[i]) {dfsinv(i, i);cout<<"Doing: " << i << endl;}
    // }
    ...
    // koseraju's does not work as expected 
    vector<int> visited1(n, false), visited2(n, false);
    function<void(int)> dfs1 = [&](int node){
        visited1[node]=true;
        for(auto x: graph[node]){
            if(!visited1[x]) dfs1(x);
        }
    };
    ...
    // all shoudl be visited 
    int a = -1, b = -1;
    for(int i=1;i<n;i++){
        if((visited1[i] && visited2[i]) == false){
            if(visited1[i] == false){
                a = 1;
                b = i+1;
            } else {
