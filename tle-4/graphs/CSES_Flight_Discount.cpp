...
void solve() {
    ...
    vector<vector<lli>> distances(2, vector<lli>(n, LLONG_MAX));

    distances[0][0] = 0;
    distances[1][0] = 0;

    set<tuple<lli,int,int>> pq;
    pq.insert({0, 0, 0});
    // pq.insert({0, 0, 1});

    while(!pq.empty()){
        auto [currDist, node, halfDone] = *pq.begin();
        pq.erase(pq.begin());

        // now update distanced 
        for(auto [v, weight]: graph[node]){
            // two distanced will be updated 
            // if halfDone is 0 -> do both
            

            if(currDist+weight < distances[halfDone][v]){
                auto it = pq.find({distances[halfDone][v], v, halfDone});
                if(it!=pq.end())pq.erase(it);
                distances[halfDone][v]=currDist+weight;
                pq.insert({distances[halfDone][v], v, halfDone});
            }

            if(halfDone==0){
                if(currDist+(weight/2) < distances[1][v]){
                    auto it = pq.find({distances[1][v], v, 1});
                    if(it!=pq.end())pq.erase(it);
                    distances[1][v]=currDist+(weight/2);
                    pq.insert({distances[1][v], v, 1});
                }
            }
