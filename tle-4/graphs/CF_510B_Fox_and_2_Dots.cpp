...
void solve() {
    ...
    // now we will do dfs from every node
    // and if during a dfs, we find another already visited element, we say we are done
    // how to cover the 4 unit distance ?

    vector<vector<bool>> visited(n, vector<bool>(m, false));

    bool ans = false;
    function<void(int,int,int,int,char)> dfs=[&](int i, int j, int pi, int pj, char color){
        if(!validate(i, j, n, m)) return;
        if(graph[i][j]!=color)return;
        if(visited[i][j]==true){
            ans=true;
            return;
        }
