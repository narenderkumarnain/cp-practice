...
void solve() {
    ...
    while(!pq.empty()){
        ...
        // cout<<currDist<<" - "<<node<<endl;
        // pq.erase(pq.begin());
        pq.pop();

        for(int i=0;i<graph[node].size();i++){
            // neighbors
            auto [v, weight] = graph[node][i];
            if(currDist+weight < distances[v]) {
                // delete previous record in pq
                // auto it = pq.find({distances[v], v});
                // if(it!=pq.end())
                //     pq.erase(pq.find({distances[v], v}));
                distances[v] = min(distances[v], currDist+weight);
                parent[v]=node;
    ...
    if(distances[n-1]==LLONG_MAX)cout << -1 << endl;
    else {
