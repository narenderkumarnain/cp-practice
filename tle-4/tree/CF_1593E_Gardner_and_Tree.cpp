...
void solve() {
    ...
    while(t--) {
        ...
        function<int(int,int)> dfs = [&](int node, int parent) -> int {
            // returns distance from leaf node
            // if multiple, return max distance 
            // if itself is leaf return 0

            int distanceFromLeaf = 0;
            for(auto x: tree[node]) {
                if(x != parent) {
                    distanceFromLeaf = max(distanceFromLeaf, dfs(x, node) + 1);
                }
            }

            distances[node] = distanceFromLeaf;
        ...
        int ans =0;
        for(int i=0;i<n;i++) if(distances[i] >= k) ans++;
        cout << ans << "\n";
    }
}
