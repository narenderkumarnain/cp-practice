...
void solve() {
    ...
    vector<lli> subtreesum(n, 0), distance(n, 0);
    function<lli(int,int)>dfs=[&](int node, int parent)->lli{
        if(parent!=-1)distance[node]=distance[parent]+1;
        lli subtreeSm = val[node];
        for(auto x:tree[node])if(x!=parent)subtreeSm+=dfs(x, node);
        subtreesum[node]=subtreeSm;
        return subtreeSm;
    };
    dfs(0,-1);
    lli ans = 0;
    for(int i=0;i<n;i++){
        ans+=distance[i]*val[i];
    }

    lli mxAns = ans;
    function<void(int,int,lli)>dfs2=[&](int node, int parent, lli currAns){
        for(auto x:tree[node]){
            if(x!=parent){
                // lli newAns=currAns;
                lli newAns = currAns - subtreesum[x] + (totalVal-subtreesum[x]);
                mxAns=max(mxAns, newAns);
                dfs2(x, node, newAns);
            }
        }
    };
