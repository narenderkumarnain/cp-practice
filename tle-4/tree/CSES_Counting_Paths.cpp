...
void solve() {
    ...
    function<void(int,int)> dfs = [&](int node, int parent) {
        ...
        for(int i=1;i<m;i++){
            if(parents[node][i-1]!=-1) parents[node][i] = parents[parents[node][i-1]][i-1];
        }

        for(auto x: tree[node]) if(parent!=x) dfs(x, node);
    };
    dfs(0, -1);

    vector<int> nodeVal(n, 0);

    // a, b -> add 1 in a to lca, then 1 in lca to b
    // difference sum technique - a + 1, b +1 , lca->parent-1, lca-1
    ...
    while(M--){
        ...
        a--;
        b--;
        int lcaVal = findLCA(a, b);

        // case when one is lca 

        nodeVal[a]++;
        nodeVal[b]++;
        nodeVal[lcaVal]--;
        if(parents[lcaVal][0]!=-1) nodeVal[parents[lcaVal][0]] -= 1;
    }

    vector<int> finalAns(n, 0);

    function<int(int,int)> dfs2 = [&](int node, int parent)->int{
        
        int ans = nodeVal[node];
