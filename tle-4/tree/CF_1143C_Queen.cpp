...
void solve() {
    ...
    function<int(int,int)> dfs = [&](int node, int parent) -> int {
        ...
        int respectCount = 1;
        if(respect[node] == 0) respectCount = 0;

        for(auto x: tree[node]) {
            if(x != parent) {
                auto res = dfs(x, node);
                respectCount &= respect[x];
            }
        }

        if(respectCount == 1) {
            // delete this node
            deleteOrder.push_back(node);
            return 0;
        }
        respectCount = 0;
        if(respect[node] == 0) respectCount=1;
        return respectCount;
    };

    dfs(root, -1);
    sort(deleteOrder.begin(), deleteOrder.end());
    for(auto x: deleteOrder) if(x != root) cout << (x+1) << " ";
    if(deleteOrder.size() == 0) cout << -1;
    cout << endl;
}
