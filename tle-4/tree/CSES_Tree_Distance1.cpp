...
void solve() {
    ...
    function<void(int,int)> dfs=[&](int node, int parent){
        ...
        for(int i=1;i<m;i++)if(parents[node][i-1]!=-1)parents[node][i]=parents[parents[node][i-1]][i-1];
        for(auto x:tree[node])if(x!=parent) dfs(x, node);
    };
    dfs(0, -1);
    ...
    int diameterNode = -1;
    for(int i=0;i<n;i++){
        if(diameterNode==-1||depth[diameterNode]<depth[i])diameterNode=i;
    }

    // now find max distance of everyone from i;
    vector<int> depth2(n, 0);
    function<void(int,int)> dfs2 = [&](int node, int parent){
        if(parent!=-1)depth2[node]=depth2[parent]+1;
        for(auto x: tree[node])if(x!=parent)dfs2(x, node);
    };
    dfs2(diameterNode, -1);
    int diameterNode2=-1;
    for(int i=0;i<n;i++){
        if(diameterNode2==-1||depth2[diameterNode2]<depth2[i])diameterNode2=i;
    }
    // cout << depth2[diameterNode2] << "some"<< endl;
    ...
    function<int(int,int)> findLCA=[&](int a, int b)->int{
        ...
        return (a==b)?a:(parents[a][0]);
    };
    // cout << diameterNode << "-" << diameterNode2 << endl;
    // we have both diameter nodes 
    for(int i=0;i<n;i++){
        // find lca -> calculate distance with both
        int lca1=findLCA(i, diameterNode);
        int dis1 = depth[i]+depth[diameterNode]-2*depth[lca1];
        // cout << lca1 << " - " << i+1 << "->"<< dis1 << endl;


        int lca2=findLCA(i, diameterNode2);
        int dist2=depth[i]+depth[diameterNode2]-2*depth[lca2];
        // cout << lca2 << " - " << i+1 << "->"<< dist2 << endl;
