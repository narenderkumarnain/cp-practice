...
void solve() {
    ...
    vector<int> distances(n, 0), subtree(n, 0);

    function<int(int,int)>dfs=[&](int node, int parent) -> int{
        if(parent!=-1)distances[node]=distances[parent]+1;
        int subtreeSize=1;
        for(auto x: tree[node])if(x!=parent){
            subtreeSize+=dfs(x, node);
        }
        subtree[node]=subtreeSize;
        return subtreeSize;
    };
    dfs(0,-1);

    lli sm = 0;
    for(auto x: distances)sm+=x;

    vector<lli>finalDis(n,0);

    function<void(int,int,lli)>dfs2=[&](int node, int parent,lli currSm){
        // make every child a parent 
        finalDis[node]=currSm;
        for(auto x: tree[node]){
            if(x!=parent){
                // make this node root
                // calculate new sum
                int childSubtreeSize=subtree[x];
                int remainingSize=n-childSubtreeSize;
                lli newsm = currSm-childSubtreeSize+remainingSize;
                dfs2(x, node, newsm);
            }
        }
    };
