...
void solve() {
    // lowest common boss of two people
    /*
    algorithm 1 -> make them same level 
    then from 1 to m, binary search if their bosses are same  FFFFFTTTTT, find first tru
    o(logn)x o(logn) => might give tle 
    */
   ...
   */
    ...
    auto query = [&](int a, int b) -> int {
        ...
        // bring all to same level 
        int depthA = depth[a];
        int depthB = depth[b];

        if(depthA < depthB) {swap(a, b);swap(depthA, depthB);}

        if(depthA != depthB){
            // whoever has more depth moved up
        ...
        // for m-1 to 0, if 2^ith parents are not equal -> move it there
        // in last, they will be at same position
        for(int i=m-1;i>=0;i--){
