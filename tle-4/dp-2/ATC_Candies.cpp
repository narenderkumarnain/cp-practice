...
void solve() {
    ...
    // dp[i][j] = no of ways to distribute j candies to children [i:n-1]

    vector<lli> dp_curr(k+1, 0), dp_prev(k+1, 0);
    dp_prev[0] = 1;

    for(lli i = n-1;i>=0;i--){
        for(int j=1;j<=k;j++){
            dp_prev[j] = mod_add(dp_prev[j], dp_prev[j-1], MOD);
        }
        ...
        for(lli j=0;j<=k;j++){
            ...
            int mxVal = j;
            int minVal = j-min(arr[i], j);

            dp_curr[j] = dp_prev[mxVal];
