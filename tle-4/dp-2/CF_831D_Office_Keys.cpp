...
void solve() {
    ...
    function<lli(int,int)> fn = [&](int idxA, int idxB) -> lli {
        if(idxA == n) {
            return 0;
        }

        if(idxB == k && idxA < n) return MAX_VAL;

        if(dp[idxA][idxB] != -1) return dp[idxA][idxB];
        lli res = MAX_VAL;

        res = min(res, fn(idxA, idxB+1));

        res = min(res, max(fn(idxA + 1, idxB + 1), (abs(a[idxA] - b[idxB]) + abs(b[idxB] - p))));
        dp[idxA][idxB] = res;
        return res;
    };
