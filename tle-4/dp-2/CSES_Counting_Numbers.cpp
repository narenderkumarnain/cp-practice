...
lli findAns(string nm) {
    int n = nm.length();

    int startIdx = 0;
    // while(startIdx < n && nm[startIdx] == '0') startIdx++;

    // find no of elements from 0 to nm which does not have adjacent digits same 

    vector<vector<vector<lli>>> dp(n, vector<vector<lli>>(2, vector<lli>(11, -1)));
    // 10 signifies leading zeros 

    function<lli(int,bool,int)> fn = [&](int idx, int bounded, int prevDigit) -> lli {
        // base cases here 
        if(idx == n) return 1;

        if(dp[idx][bounded][prevDigit] != -1) return dp[idx][bounded][prevDigit];
        lli ans = 0;

        for(char c = '0'; c <= ((bounded == 1) ? nm[idx] : '9'); c++){
            // construct ans with that
            if((c-'0') == prevDigit){
                continue;
            } else {
                int newPrevDigit = c-'0';
                if(prevDigit == 10 && c == '0') newPrevDigit = 10;
                ans += fn(idx+1, bounded && (c == nm[idx]), newPrevDigit);
            }
        }
        dp[idx][bounded][prevDigit] = ans;
        return ans;
    };
    

    return fn(startIdx, 1, 10);
}
...
void solve() {
    ...
    lli ans = findAns(b) - findAns(a);

    // cout << findAns(b) << endl;
    // check conditionn for a 
    int idx = 0;
    int n = a.length();
    while(idx < n && a[idx] == '0') idx ++;
    bool valid = true;
    for(int start=idx;start<n;start++){
        if(start > idx && a[start] == a[start-1]) {
            valid = false;
            break;
        }
    }
