...
void solve() {
    ...
    vector<vector<bool>> dp(n, vector<bool>(100000+1, false));

    set<int> st;
    function<void(int, int)> fn  = [&](int idx, int prevSum) -> void{
        if(idx == n) return;

        if(dp[idx][prevSum] == true) return;

        st.insert(prevSum + x[idx]);

        fn(idx+1, prevSum + x[idx]);
        fn(idx+1, prevSum);
