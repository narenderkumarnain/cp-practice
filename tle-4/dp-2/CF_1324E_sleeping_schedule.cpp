...
void solve() {
    ...
    // dp[i][j] = no of good sleeps when ith sleep ends on jth hour of day
    // final subproblem = dp[n][any]

    for(int i=1;i<=n;i++) {
        for(int j=0;j<h;j++){
            dp[i][j] = max(dp[i][j], dp[i-1][mod_sub(j,a[i-1], h)]);
            dp[i][j] = max(dp[i][j], dp[i-1][mod_sub(j,a[i-1]-1, h)]);

            if(j >= l && j <= r && dp[i][j]!=INT_MIN) dp[i][j]++;
        }
    }

    int finalAns = 0;
    for(int i=0;i<h;i++) finalAns = max(finalAns, dp[n][i]);
    cout << finalAns << endl;
}
