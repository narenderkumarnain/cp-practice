...
void solve() {
    ...
    int N = 3000;
    vector<lli> dp_curr(N+1, 0), dp_prev(N+1, 1);

    for(int i=n-1;i>=0;i--){
        // for(auto x: dp_prev) cout << x << ", ";

        // prefix sum 
        for(int j=1;j<=N;j++) {
            dp_prev[j] = (dp_prev[j-1] + dp_prev[j]) % MOD;
        }

        
        for(lli j=0;j<=N;j++) {
            lli minVal = max(a[i], j);
            lli maxVal = b[i];

            if(minVal > maxVal) {
                // not result is possible
                dp_curr[j] = 0;
            } else {
                dp_curr[j] = dp_prev[maxVal];
                if(minVal > 0) dp_curr[j] = mod_sub(dp_curr[j], dp_prev[minVal-1], MOD);
            }
        }
