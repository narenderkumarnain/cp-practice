...
void solve() {
    ...
    // O(n) * O(log(1e9)) -> 1e8 solution gave TLE
    // vector<unordered_map<int,int>> dp(n);
    // // brute force recursive solution
    // function<int(int,int)> recurFn = [&](int i, int val) -> int {
    //     // choose i or no choose i
    //     int finalRes = 0;
    ...
    //     if(arr[i] > val) {
    //         // choose it
    //         finalRes = max(finalRes, 1 + recurFn(i+1, arr[i]));
    //     }

    //     finalRes = max(finalRes, recurFn(i+1, val));

    //     dp[i][val] = finalRes;
    //     return finalRes;
    // };

    // cout << recurFn(0, INT_MIN) << endl;

    // dp based solution again
