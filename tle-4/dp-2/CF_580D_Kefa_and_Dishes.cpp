...
void solve() {
    ...
    vector<vector<lli>> grid(n+1, vector<lli>(n+1, 0));
    for(int i=0;i<k;i++) {
        int x, y, c;
        cin >> x >> y >> c;
        grid[x][y] = c;
    ...
    vector<vector<lli>> dp((1 << (n+1))-1, vector<lli>(n+1, -1));

    function<lli(int,int)> fn = [&](int visited, int prevEat) -> lli {
        lli ans = INT_MIN;
        int totalEaten = __builtin_popcount(visited);
        if(totalEaten == m){
            return 0;
        }
        // if(idx == n){
        //     return INT_MIN; // think about it 
        //     // failure case here 
        // }
        if(dp[visited][prevEat] != -1) return dp[visited][prevEat];
        // eat any that is available
        for(int j=0;j<n;j++) {
            if((visited & (1 << j)) == 0) {
                // available 
                lli sm = 0;
                sm += a[j];
                sm += grid[prevEat][j+1];
                lli tmpres = fn(visited | (1 << j), j+1);
                if(tmpres == INT_MIN){
                    continue;
                }
                ans = max(ans, sm + tmpres);
            }
        }
        dp[visited][prevEat] = ans;
