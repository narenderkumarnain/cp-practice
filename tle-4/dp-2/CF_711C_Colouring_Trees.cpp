...
void solve() {
    ...
    vector<vector<lli>> paint(n, vector<lli>(m));
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++) cin >> paint[i][j];
    }


    vector<vector<vector<lli>>> dp(n, vector<vector<lli>>(m+1, vector<lli>(k+1, -1)));

    function<lli(int,int,int)> fn = [&](int idx, int prevColor, int groups) -> lli {
        // comeback to it
        if(groups > k) return MAX_ANS;
        if(idx == n && groups == k) {
            return 0;
        }
        if(idx == n) return MAX_ANS;

        // cout << idx << " " << prevColor << " " << groups << endl;
        if(prevColor != -1 && dp[idx][prevColor][groups] != -1) return dp[idx][prevColor][groups];

        lli ans = MAX_ANS;
        if(c[idx] != 0) {
            if(c[idx] == prevColor) {
                ans = min(ans, fn(idx+1, prevColor, groups));
            } else {
                ans = min(ans, fn(idx+1, c[idx], groups+1));
            }
        } else {
            // we can color this out
            for(int color=1;color<=m;color++) {
                int newGroups = groups;
                if(color != prevColor) newGroups++;
                ans = min(ans, fn(idx+1, color, newGroups) + paint[idx][color-1]);
            }
        }
        if(prevColor != -1) dp[idx][prevColor][groups] = ans;
        return ans;
    };
