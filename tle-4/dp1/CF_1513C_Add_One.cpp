...
void solve() {
    ...
    int M = 2e5 + 1;
    vector<vector<lli>> dp(M+1, vector<lli>(2, 0));
        // dp[10][0] = 1;
        // dp[9][1] = 1;

    for(int i=0;i<=M;i++) {
        if(i-10 >= 0) dp[i][0] = (1 + dp[i-10][1] + dp[i-10][0])%MOD;
        if(i-9>=0) dp[i][1] = (1 + dp[i-9][1] + dp[i-9][0])%MOD;
    }
    ...
    while(t--) {
        ...
        while(tmp > 0) {
            ...
            int digit = tmp % 10;
            tmp = tmp / 10;
            if(m-(10-digit) >= 0) ans = (ans + 1 + dp[m-(10-digit)][0] + dp[m-(10-digit)][1]) % MOD;
        }
