...
// lli countWays(int n, int l, int e, int m){
//     if(n == 1) {
//         if(e == -1) {
//             return 3;
//         }
...
//         lli cnt = 0;
//         if(abs(e-l) <= 1 && l <= m && l >= 1) cnt++;
//         if(abs(e-(l-1)) <= 1 && (l-1) >= 1) cnt++;
//         if(abs(e-(l+1)) <= 1 && l+1 <= m) cnt++;
...
//     cnt = mod_add(cnt, countWays(n-1, l, e, m), MOD);

//     if(l-1 >= 1) cnt = mod_add(cnt, countWays(n-1, l-1, e, m), MOD);
//     if(l+1 <= m) cnt += mod_add(cnt, countWays(n-1, l+1, e, m), MOD);
...
void solve() {
    ...
    for(int i=0;i<n;i++) if(arr[i] != 0) dp[i][arr[i]] = 1;
    ...
    for(int i=0;i<n;i++) {
        ...
        for(int j=1;j<=m;j++) {
            ...
            if(i > 0) {
                sm = mod_add(sm, dp[i-1][j], MOD);
                if(j-1 >= 0) sm = mod_add(sm, dp[i-1][j-1], MOD);
                if(j+1 <= m) sm = mod_add(sm, dp[i-1][j+1], MOD);
            }
    ...
    if(lastIdx == -1) cout << 1 << endl;
    else {
        lli cnt = 0;
        for(int j=1;j<=m;j++) {
            cnt = mod_add(cnt, dp[lastIdx][j], MOD);
        }
        cout << cnt << endl;
    }
}
