...
void solve() {
    ...
    vector<int> leftDiagonals1(2 * 1000 + 1, 0);
    vector<int> rightDiagonal(2 * 1000 + 1, 0);
    ...
    for(int i=0;i<=2*1000;i++) {
        if(leftDiagonals1[i] > 1) {
            ans += mth.choose(leftDiagonals1[i], 2);
        }
        if(rightDiagonal[i] > 1) {
            ans += mth.choose(rightDiagonal[i], 2);
        }
    }
