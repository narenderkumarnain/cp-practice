...
void solve() {
    ...
    vector<int> independent, red, blue, both;
    for(int i=0;i<n;i++) {
        string s;
        cin >> s;
        int a;
    ...
    sort(independent.begin(), independent.end(), greater<int>());
    sort(red.begin(), red.end(), greater<int>());
    sort(blue.begin(), blue.end(),greater<int>());
    sort(both.begin(), both.end(),greater<int>());

    int idxi = 0, idxr = 0, idxb = 0, idxboth = 0;

    int minCore = min(red.size(), blue.size());

    lli ans = 0;

    int req = minCore;
    while(idxr < red.size() && req > 0) {
        // cout << "choose red: " << red[idxr] << endl;
        ans += red[idxr];
        idxr++;
        req--;
    }
    ...
    req = minCore;
    while(idxb < blue.size() && req > 0) {
        ans += blue[idxb];
        idxb++;
        req--;
    }

    int extraCount = both.size();
    while(idxboth < both.size()) {
        ans += both[idxboth];
        idxboth++;
    }

    // now we have allowed extras

    while(extraCount > 0) {
        // see if you can assign anyone 
        int maxVal = 0;
        int source = -1;
        ...
        if(idxi < independent.size()) {
            if(independent[idxi] > maxVal) {
                maxVal = independent[idxi];
                source = 0;
            }
        }
