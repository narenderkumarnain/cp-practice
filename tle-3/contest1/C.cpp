...
void solve() {
    ...
    while(t--) {
        ...
        while(idx < n) {
            ...
            // currSame = 1;
            uniqueArr.push_back(arr[idx]);
            idx++;
        }

        // int start = 0;
        // int end = uniqueArr.size();
        int nn = uniqueArr.size();

        // int currAns = 0;
        // while(start <= end) {
        //     int mid = start + (end-start)/2;

        //     // mid is the size of subarray
        // }
        int a = 0, b = 0;
        int ans = 0;
            while(b < nn) {
                // add b to computation
                while(a < b && uniqueArr[b]-uniqueArr[a]>=n) {
                    a++;
                }

                if(uniqueArr[b]-uniqueArr[a] < n) {
                    ans = max(ans, b-a+1);
                }
                b++;
            }
