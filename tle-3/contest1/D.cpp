...
void solve() {
    ...
    vector<pair<int,int>> seqa(n), seqb(m);
    for(int i=0;i<n;i++) {
        int a, b;
        cin >> a >> b;
        seqa[i] = {a,b};
    ...
    while(idxa < seqa.size() && idxb < seqb.size()) {
        auto t1 = seqa[idxa];
        auto t2 = seqb[idxb];

        /**
         * if no overlap -> increment the one with earlier time range
         * else -> add the common overlap time duration
         * then increment the one which ends earlier 
         */
        ...
        if(
            ...
            if(t1.second <= t2.second) idxa++;
            else idxb++;
        } else {
            lli overlapAmount = min(t1.second, t2.second) - max(t1.first, t2.first);
