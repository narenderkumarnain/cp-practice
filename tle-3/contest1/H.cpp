...
bool palindromeCheck(vector<int> &mapa, vector<int>& mapb) {
    int oddCount = 0;
    for(int i=0;i<26;i++) {
        int sm = mapa[i] + mapb[i];
        if(sm % 2 != 0) oddCount++;
    }

    if(oddCount == 0 || oddCount == 1) return true;
    return false;
}
...
void solve() {
    ...
    int ans = 0;
    vector<vector<int>> freqMaps(n, vector<int>(26));
    ...
    for(int i=0;i<n;i++) {
        ...
        // cout << "non palindrome " << nonPlaindrome << endl;
        unordered_map<string, vector<int>> toAdd;
        if(noOfOdds == 0 || (s.length()%2==1 && noOfOdds == 1)) {
            ans += palindromeCount;
            palindromeCount++;
            if(nonPlaindrome.length() > 0) toAdd[nonPlaindrome].push_back(i);
        } else {
            // non palindromic part

            unordered_set<int> done;

            for(int k=0;k<nonPlaindrome.length();k++) {
                // remove this character only and check if completed
                string key = nonPlaindrome.substr(0, k);
                if(k < n-1) key += nonPlaindrome.substr(k+1, nonPlaindrome.length()-k-1);
                // cout << "key: " << key << endl;
                for(auto candidate: mp[key]) {
                    if(palindromeCheck(freqMaps[i], freqMaps[candidate]) && done.find(candidate) == done.end()) {ans++;
                        // cout << "found: " << i << " " << candidate << endl;
                        done.insert(candidate);
                    }
                }

                toAdd[key].push_back(i);
            }

            for(auto candidate: mp[nonPlaindrome]) {
                if(palindromeCheck(freqMaps[i], freqMaps[candidate]) && done.find(candidate) == done.end() ) {ans++;
                // cout << "found: " << i << " " << candidate << endl;
                done.insert(candidate);
                }
            }
            toAdd[nonPlaindrome].push_back(i);  
        }
