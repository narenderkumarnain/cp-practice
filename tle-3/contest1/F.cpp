...
void solve() {
    lli n;
    cin >> n;
    lli start = 1, end = n, ans = n;

    lli priyanshReq = (n + 1) / 2;

    while (start <= end) {
        lli k = start + (end - start) / 2;

        lli currCount = n;
        lli priyanshCount = 0;

        while (currCount > 0) {
            lli toPriyansh = min(k, currCount);
            priyanshCount += toPriyansh;
            currCount -= toPriyansh;
            if (priyanshCount >= priyanshReq) break;

            if (currCount >= 10) currCount -= currCount / 10;
        }

        if (priyanshCount >= priyanshReq) {
            ans = k;
            end = k - 1;
        } else {
            start = k + 1;
