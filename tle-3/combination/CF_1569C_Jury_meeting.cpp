...
int main() {
    ...
    while(t--) {
        ...
        int greatestFreq = 0;
        int nextGreatest = -1;
        int nextGreatestFreq = 0;
        greatestElement = arr[0];
        while(idx < n && arr[idx] == greatestElement) {idx++; greatestFreq++;}

        if(idx == n || greatestFreq > 1) {
            cout << mth.fact[n] << endl;
        } else {
            // greatest freq is 1
            nextGreatest = arr[idx];
            while(idx < n && arr[idx] == nextGreatest) {idx++; nextGreatestFreq++;}

            if(greatestElement - nextGreatest > 1) {
                cout << 0 << endl;
            } else {
                lli tmp = mth.fact[n];
                tmp = mod_sub(tmp, mth.fact[n-1], MOD);

                int noOfSmallEle = n - greatestFreq - nextGreatestFreq;

                for(int i=1;i<=noOfSmallEle;i++) {
                    lli tmp2 = mth.choose(noOfSmallEle, i);
                    tmp2 = mod_mul(tmp2, mth.fact[i], MOD);
                    tmp2 = mod_mul(tmp2, mth.fact[n - i - 1], MOD);

                    tmp = mod_sub(tmp, tmp2, MOD);
                }
