...
int main() {
    ...
    for(int i=0;i<s2.length();i++){
        if(s2[i] == '+') currMoves++;
        else if(s2[i] == '-') currMoves--;
        else unknownMoves++;
    }

    int diff = currMoves - netMove;
    if(diff == 0) {
        if(unknownMoves == 0)
            cout << 1.0 << endl;
        else {
            if(unknownMoves % 2 == 0) {
                long double tmp = pow((double)0.5,(double)unknownMoves);
                tmp = tmp * ((long double)mth.choose(unknownMoves, unknownMoves/2));
        ...
        diff = abs(diff);
        if(diff > unknownMoves) cout << 0.0 << endl;
        else if(diff == unknownMoves) {
            cout << setprecision(10) << fixed << pow((double)0.5,(double)unknownMoves) << endl;
        }
        else {
            if(abs(diff-unknownMoves) %2 != 0) {
                cout << 0.0 << endl;
            }else{
                long double tmp = pow((double)0.5,(double)unknownMoves);
                tmp = tmp * ((long double)mth.choose(unknownMoves, abs(diff-unknownMoves)/2));
                cout << setprecision(10) << fixed << tmp << endl;
                // cout << setprecision(10) << fixed << pow((double)0.5,(double)unknownMoves) << endl;
            }
        }
    ...
    // long double tmp = pow((double)0.5,10);
    // tmp = tmp * ((long double)mth.choose(10, 4));
    // cout << setprecision(10) << fixed << tmp << endl;
    return 0;
}