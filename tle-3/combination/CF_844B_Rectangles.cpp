...
int main() {
    ...
    for(int i=0;i<n;i++) {
        // for each row 
        lli noOfBlack = rowCount[i];
        lli noOfWhite = m - rowCount[i];
    ...
    for(int j = 0;j<m;j++) {
        lli noOfBlack = columnCount[j];
        lli noOfWhite = n - columnCount[j];

        res += (1LL << (noOfBlack)) - 1 - noOfBlack;
        res += (1LL << (noOfWhite)) - 1 - noOfWhite;
    }
