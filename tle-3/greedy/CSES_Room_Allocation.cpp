...
int main() {
    ...
    for(int i=0;i<n;i++) {
        if(!pq.empty() && pq.top().first < customers[i].first.first) {
            ...
            ans[customers[i].second] = curr.second;

            pq.push({ customers[i].first.second, curr.second });
        } else {
            noOfRooms++;
            lastRoomIdx++;

             ans[customers[i].second]  = lastRoomIdx;

            pq.push({ customers[i].first.second, lastRoomIdx});
        }
    }
