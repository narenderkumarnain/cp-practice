...
void solve() {
    ...
    while(t--) {
        ...
        vector<deque<int>> dq(10);
        int n = num.length();

        for(int i=0;i<n;i++) {
            dq[num[i] - '0'].push_back(i);
        }
        ...
        int idx = 0;
        while(ans.length() < (n-k) && currK > 0) {
            int start=0;
            if(ans.length() == 0) start = 1;

            int validMinIdx = -1;
            // int validMinVal = 10;
            for(;start<10;start++) {
                if(dq[start].empty()) continue;
                int newIdx = dq[start].front();
                if((newIdx-idx) <= currK) {
                    // cout << "currmath: " << newIdx  << " - " << idx << endl;
                    validMinIdx = newIdx;
                    break;
                }
            }

            // cout << "validMinIdx: "<< validMinIdx << endl;

            ans += num[validMinIdx];
            for(int j=0;j<10;j++) {
                while(!dq[j].empty() && dq[j].front() <= validMinIdx) {dq[j].pop_front();}
            }
            currK -= validMinIdx - idx;
            idx = validMinIdx+1;
        }

        // cout << "temp ans: " << ans << "idx: " << idx << endl;

        while(ans.length() < (n-k)) {
            // bool conti = true;
            for(int i=0;i<10;i++) {
                if(!dq[i].empty() && dq[i].front() == idx) {
                    ans += num[dq[i].front()];
                    dq[i].pop_front();
                    idx++;
                    break;
