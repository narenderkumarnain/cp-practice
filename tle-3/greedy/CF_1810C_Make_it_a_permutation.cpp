...
void solve() {
    ...
    while(t--) {
        ...
        while(idx < n) {
            ...
            lli currN = val;
            lli missingEle = currN - uniqueCount;

            lli extraElements = (n-idx-1);
            if((idx+1) > uniqueCount) {
                extraElements += (idx-uniqueCount+1);
            }

            // cout << idx << " - " << missingEle << " , " << extraElements << endl;

            lli ans = (c*extraElements) + (d * missingEle);
            minAns = min(minAns, ans);
            
            while(idx < n && arr[idx] == val) idx++;
        }

        minAns = min(minAns , (c * n) + d);
