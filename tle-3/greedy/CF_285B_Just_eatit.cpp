...
void solve() {
    ...
    while(t--) {
        ...
        for(int i=0;i<n;i++) {
            if((runningsumstart == 0) && (i == n-1) && runningSum+arr[i] > arr[i]) {
                continue;
            }
            // int newStart = arrStart;
            // int newEnd = arrEnd;
            // int runningsumstartNew = runningsumstart;
            ...
            runningSum = max(runningSum + arr[i], arr[i]);
            if(runningSum > mxSum) {
                // arrStart = newStart;
                // arrEnd = newEnd;
                mxSum = max(mxSum , runningSum);
            }
        }
