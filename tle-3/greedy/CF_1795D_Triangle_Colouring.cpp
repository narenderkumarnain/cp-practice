...
void solve() {
    ...
    for(int i=0;i<n/3;i++){
        ...
        lli noOfWays = 0;
        if(a == minEle) noOfWays++;
        if(b == minEle) noOfWays++;
        if(c == minEle) noOfWays++;

        // cout << noOfWays << endl;

        noOfWaysToMakeMax = mod_mul(noOfWaysToMakeMax, noOfWays, MOD);
    }

    Math<lli> mth(n+1);

    lli noOfWaysToColor = mth.choose(n/3 , n/6);
    // cout << noOfWaysToMakeMax << " - " << noOfWaysToColor << endl;
    noOfWaysToMakeMax = mod_mul(noOfWaysToMakeMax, noOfWaysToColor, MOD);

    // noOfWaysToMakeMax = mod_mul(2, noOfWaysToMakeMax, MOD);

    cout << noOfWaysToMakeMax << "\n";
}
